...
if ($preservedXval) {
    ...
    $lines.Add('')
    $lines.Add('External cross-check via HWiNFO64 v8.14-5580 -- see [`screenshots/LNL_hwinfo_memory_clocks.png`](../screenshots/LNL_hwinfo_memory_clocks.png) for the captured Sensors view (host: ASUS UX5406SA / Core Ultra 7 258V).')
    $lines.Add('')
    $lines.Add('| Source                                    | Reported value | Notes |')
    $lines.Add('|---|---|---|')
    $lines.Add(('| Module diagnostic IOCTL (locked max)      | QCLK = {0:N1} MHz, data rate = {1:N0} MT/s, IO = {2:N0} MHz | derived as ratio={3} x BCLK/3 x gear={4} |' -f $qclkMHz, $mtps, $ioMHz, $pmaRatio, $pmaGear))
    $lines.Add( '| Module static IOCTL                       | QCLK = 2,133.3 MHz, Flags = 0x1 (`STATIC_LOCKED`) | gate now open on LNL; matches the diagnostic decode |')
    $lines.Add( '| Module live IOCTL (single sample)         | QCLK = 2,133.3 MHz, Gear = Gear4 | live; live workpoint range in the time-series tables above |')
    $lines.Add( '| HWiNFO64 Memory Clock (cur/min/max/avg)   | 2,133.3 / 600.0 / 2,133.3 / 2,032.0 MHz | matches QCLK exactly (= ratio 64 x 33.333 MHz at max, ratio 18 x 33.333 MHz at min) |')
    $lines.Add( '| HWiNFO64 Memory Clock Ratio (cur/min/max) | 21.33x / 6.00x / 21.33x | equals module ratio / 3 (HWiNFO uses BCLK as the reference, the module uses BCLK/3) |')
    $lines.Add( '| HWiNFO64 Gear Mode                        | 2 | **does not match** the module''s `Gear4` decode -- see note below |')
    $lines.Add(('| Win32_PhysicalMemory.ConfiguredClockSpeed | {0:N0} MT/s (per DIMM, 8 x 4 GiB on-package) | matches the data rate above |' -f ($dimms[0].ConfiguredClockSpeed)))
    $lines.Add('')
